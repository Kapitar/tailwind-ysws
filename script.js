...
async function fetchData() {
  ...
  const response = await fetch(
    `http://${BASE_DOMAIN}/api/submissions?${params}`
  );
  ...
  submissions.submissions.forEach((submission) => {
    ...
    submissionsPush += `<div class="py-6 px-8 bg-blue-700 rounded-xl shadow-lg">
          <div
          ...
          <p class="text-sm border-t border-blue-600 pt-4">Submitted on ${date.toLocaleDateString("en-US")}</p>
        </div>`;
  });
